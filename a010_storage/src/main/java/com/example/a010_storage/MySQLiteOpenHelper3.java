package com.example.a010_storage;
...
public class MySQLiteOpenHelper3 extends SQLiteOpenHelper {
    ...
    //앱설치후 최초에 데이터베이스가 '없는경우', 데이터베이스 생성을 위해 호출되는 콜백
    ...
    //데이터베이스의 '버젼'이 바뀌었을 때 호출되는 콜백 메소드
    //'버전' 이 바뀌었을때 기존에 설치 운영되고 있는 데이터베이스를 어떻게 변경할 것인지 작성
    // 각 버젼의 변경 내용들을 버젼마다 작성해야함.

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        Log.d("myapp", "SQLiteOpenHelper] onCreate() 호출"+oldVersion + "->"+newVersion);
