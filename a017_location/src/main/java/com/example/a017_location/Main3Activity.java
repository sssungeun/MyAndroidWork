...
public class Main3Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
               ...
               String str = etAddress.getText().toString();
                ...
                if (list != null) {
                    if (list.size() == 0) {
                        ...
                        for(Address addr: list){
                            result.append("-------------------\n");
                            result.append(addr.getCountryName()+","+addr.getFeatureName()+","+addr.getLocality()+
                                    addr.getLatitude()+","+
                                    addr.getLongitude()+"\n");
                        }
        ...
        });//end btnGeoCoder2.seton

        btnMap1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //위도, 경도 입력 후 지도 버튼 클릭 -> 지도 앱 화면으로 묵시적 인텐트
                double lat = Double.parseDouble(etLatitude.getText().toString());
                double lng = Double.parseDouble(etLongitude.getText().toString());
